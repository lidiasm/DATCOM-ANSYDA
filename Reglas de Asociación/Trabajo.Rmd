...
2. La mayoría de los entrevistados son de **nacionalidad india**.
...
1. Existe una mayor representación de **encuestados indecisos al pedir postre** puesto que la frecuencia de este ítem es superior a las otras dos opciones, por lo que puede ser un caso de estudio particular para intentar eliminar sus dudas y fomentar el consumo.
...
3. El momento del día en el que existe una mayor afluencia de entrevistados es el **intervalo de tarde** de 13 PM a 18 PM. **Su frecuencia es considerablemente alta** con respecto a los restantes rangos, los cuales son más cercanos entre sí, por lo que existe prácticamente el mismo número de encuestados que han visitado restaurantes por la mañana, de noche y de madrugada. Este hecho nos indica que existe una clara preferencia por este horario según los encuestados. 

4. Finalmente podemos apreciar que entre los 20 ítems más frecuentes **apenas aparecen algunas nacionalidades** diferentes a la mayoritaria. En la parte inferior del gráfico se encuentran algunos ítems que involucran a entrevistados procedentes de Malasia, Indonesia y Pakistán. Este hecho nos indica que existe poca información sobre la mayoría de nacionalidades por lo que será bastante complicado obtener relaciones entre ellas y el resto de ítems.
...
Para intentar reducir el número de itemsets frecuentes (305) procedemos a imponer una restricción adicional con la que obtener únicamente aquellos cuyos superconjuntos inmediatos son frecuentes: los **itemsets cerrados**. Como podemos apreciar, hemos conseguido un número menor de itemsets que en el caso anterior aunque siguen siendo demasiados. De hecho, si los ordenamos por su soporte en orden decreciente, podemos observar que los seis primeros **son los mismos itemsets frecuentes mostrados anteriormente**.
...
```
Es tan mínima la diferencia entre los itemsets frecuentes y cerrados que se han obtenido de este dataset, que el siguiente gráfico, donde se representan las frecuencias de sus longitudes, es prácticamente idéntico al anterior. Por lo tanto, entre la composición de los **itemsets frecuentes y cerrados apenas se pueden apreciar diferencias**.
...
```
En el siguiente diagrama de barras podemos apreciar un **aumento de la longitud de los itemsets maximales**, como hemos apuntado anteriormente, siendo los más predominantes aquellos compuestos por 4 y 5 variables. Esto nos puede indicar que la reducción de la cantidad de itemsets tiene como consecuencia el **aumento del número de variables que intervienen** en la explicación de las diferentes combinaciones que pueden surgir dentro del dataset.
...
El primer algoritmo que procedemos a aplicar es **Apriori** para la extracción de reglas con un **soporte mínimo de 0.1 y una confianza mínima de 0.8**. Adicionalmente imponemos la restricción de que cada regla disponga al menos de dos componentes, el antecedente y el consecuente. Como podemos observar en el resumen mostrado a continuación, esta técnica ha logrado generar un total de **338 reglas con longitudes de entre 2 y 6 ítems**, aunque la mayoría de ellas contienen 4 ítems concretamente. En relación a las medidas de calidad que se calculan por defecto, podemos observar que la **mayoría de reglas disponen de un soporte mínimo**, puesto que apenas existe diferencia entre el Q1 y Q3. Este hecho nos indica que existe una concentración de reglas que **representan dependencias y relaciones entre ítems poco frecuentes**, lo cual puede ser interesante para este estudio. Por otro lado podemos observar que el primer y tercer cuartil de la medida de interés o *lift* son mayores que uno, lo que puede reflejar la existencia de **dependencias positivas** entre los diferentes ítems.
...
Unificando la información extraída anteriormente y agrupando las diferentes reglas generadas con el algoritmo Apriori y que han parecido ser más relevantes, a continuación realizamos un resumen de las hipótesis obtenidas.
...
2. También hemos podido observar que los **comensales de origen indio** disponen de cierta preferencia por el **menú tradicional**. Sin embargo, debemos de considerar que tanto el tipo de comida como la nacionalidad son valores mayoritarios en este dataset por lo que se trata de información fácilmente inferible a partir del análisis exploratorio de los datos.
...
Finalmente generamos el último conjunto de reglas mediante el algoritmo FP-growth estableciendo la variable **`Juice` como consecuente**. Tras filtrar según la confianza y la medida de calidad *lift* como anteriormente, podemos apreciar la existencia de dos reglas cuya información ya es conocida. La primera afirma que durante el **intervalo de mañana se consume jugo fresco**. Esta preferencia ya se había descubierto puesto que entre las 6 AM y las 12 AM se consume el menú tradicional que suele estar acompañado de esta bebida. Mientras que la segunda regla indica que un comensal **no pide postre si solicita zumo**. De nuevo esta información tampoco es novedosa puesto que anteriormente hemos comprobado cómo los entrevistados no solicitan el tercer plato si consumen el menú tradicional con zumo. Por lo tanto, **ambas reglas parecen ser despreciables**.
...
En este subapartado se pretende actualizar las conclusiones obtenidas a partir de los análisis realizados a los conjuntos de reglas obtenidos con los algoritmos Apriori y FP-growth.

1. En primer lugar podemos determinar que la comida tradicional con zumo se suele servir durante los intervalos de mañana y noche, mientras que la comida oriental está más solicitada durante la tarde. Por lo tanto ya disponemos de un posible **patrón que relaciona el tipo de menú con el momento del día** en el que se prefiere consumir.

2. Adicionalmente parece que si se opta por el **menú tradicional no se consume postre**. Como hemos comentado anteriormente, si se trata de desayunos es lógico pensar que los comensales no solicitan este tercer plato. Sin embargo, al aparecer también existe esta situación en las cenas, por lo que puede suponer un caso de **estudio específico para fomentar su consumo** durante esta comida. Si recordamos una de las hipótesis obtenidas con el primer algoritmo indicaba que las **mujeres adultas son más indecisas**. Por lo tanto, podemos intuir que es probable que sea el género opuesto, el masculino, sea el que solicite o rechace sin dudas el consumo de postre.

## Inclusión de ítems negativos

Tras realizar diversas extracciones de reglas mediante la aplicación de los algoritmos Apriori y FP-growth sobre el dataset preprocesado, a continuación presentamos una reproducción de los análisis anteriores con la **inclusión de ítems negativos**. El proceso para su generación es similar a la binarización de una variable, en el que se crea una nueva columna por cada posible valor y se indica la existencia de cada uno mediante valores binarios. Una de las variables que más se prestan al significado de los ítems negativos es la columna **`Food`**, que representa si una persona escoge un menú o el contrario. Mientras que otra de las columnas más propensas a esta experimentación podría ser **`Dessert`** puesto que contiene las tres posibles decisiones en relación al consumo de postre tras la comida principal.
...
El último algoritmo que se pretende incluir en este trabajo se conoce como **MOPNAR**. Esta técnica es capaz de extraer reglas de asociación considerando tanto valores numéricos como categóricos. A diferencia de las técnicas anteriores, el algoritmo MOPNAR puede generar por sí mismo **intervalos que discretizan las variables numéricas, además de generar ítems negados** en caso de considerarlo útil. Por ello, creamos un nuevo dataset con los datos preprocesados y la variable `Age` original con el objetivo de que se a el algoritmo quien confeccione sus intervalos. Como en los análisis anteriores, ordenamos y filtramos las reglas a través de medidas de calidad como la confianza y el *lift*. Según los resultados obtenidos podemos observar que el **conjunto generado es mucho más numeroso** que con las pasadas técnicas. Adicionalmente, tanto la **longitud de las reglas, como sus confianzas y *lifts* han aumentado considerablemente**. Estas características pueden indicar que se trata de reglas más certeras y diversas al estar compuestas por un mayor número de combinaciones de ítems.

Las cuatro primeras reglas en combinación con las número 8, 9 y 12 se pueden analizar en conjunto para representar la siguiente hipótesis: si un **hombre malasio no solicita comida oriental pero sí un refresco, con la posibilidad de postre**, es probable que tenga entre 19 y 21 años. Así, este conjunto es capaz de identificar un nuevo patrón que asocia una nacionalidad, de la que hasta el momento no se conocía información, con un intervalo de edad generado por el algoritmo. 

Las reglas 5 y 6 apoyan una teoría del consumo de **comida oriental con refresco durante el intervalo de tarde**. Adicionalmente, esta hipótesis también proporciona información acerca del tipo de comensal que solicita este menú, como su edad que parece ser **menor de 31 años** si recordamos que el máximo valor es 80, o la **posibilidad de pedir postre**. Si incluimos en este conjunto a la regla número 19, con una menor confianza podemos especificar aún más la posible edad de los comensales que prefieren este tipo de comida: **menores de 27 años**. Así, agrupando este conjunto de tres reglas podemos detallar una hipótesis obtenida en análisis anteriores añadiendo variables tales como el rango de edad o la tendencia a solicitar un tercer plato.

Finalmente si consideramos aquellas reglas con **mayores soportes**, como las número 14, 15, 17 y 18, podemos apreciar que respaldan la teoría de la **afluencia de comensales** indios en cualquier momento del día, **excepto en el intervalo de tarde**. Por lo tanto aumenta el valor de la dependencia entre esta nacionalidad y el momento del día en el que acuden a los restaurantes. Asimismo, también refuerza la probabilidad de que opten por el **menú tradicional con zumo**. Si estas condiciones se cumplen, entonces es bastante posible que estos clientes se encuentren en un **intervalo de edad entre 23 y 50 años**. De este modo, como en el caso anterior, añadimos más detalles a las hipótesis conocidas a partir de los algoritmos anteriores.

```{r}
# Construimos un nuevo dataset con las columnas preprocesadas anteriormente
# excepto la variable `Age`
food.mopnar.df <- food.prep.df %>% select(-Age)
# Añadimos la variable `Age` original descartando las filas que tenían valores
# perdidos en la columna `Gender`
food.mopnar.df$Age <- food.df$Age[-c(which(food.df$Gender == ""))]
# Cargamos las librerías necesarias para trabajar con el algoritmo MOPNAR
require(RKEEL)
library(RKEEL)
# Creamos una instancia del algoritmo proporcionando el nuevo dataset preprocesado
food.mopnar.alg <- MOPNAR_A(food.mopnar.df)
# Ejecutamos el algoritmo para extraer las reglas
food.mopnar.alg$run()
# Ordenamos las reglas por confidence
food.mopnar.alg$sortBy("confidence")
# Mostramos las 10 primeras reglas con sus medidas de calidad
inspect(subset(food.mopnar.alg$rules, subset=confidence >= 0.8 & lift >= 1.1))
```

# Conclusiones finales

Tras realizar diversos análisis con diferentes algoritmos y considerar distintas métricas de calidad, a continuación vamos a intentar responder a las preguntas objetivo que se plantearon al comienzo de este proyecto. 

**1. ¿Existe algún tipo de patrón alimenticio dependiendo del momento del día? Considerando tanto el tipo de comida, bebida y la inclusión de postre.**

Según hemos podido observar en la mayor parte de los conjuntos de reglas obtenidos, parece que existe una preferencia por la **comida tradicional acompañada de zumo sin postre durante las mañanas y las noches**. Por lo tanto podría tratarse de menús relacionados con desayunos, tentempiés y cenas. En este último tipo de comida es donde se puede orientar futuros estudios para promover el consumo de postre, ya que en las restantes comidas no parece muy lógico. Mientras tanto, la **comida oriental** parece ser más solicitada **durante la tarde**, seguramente como almuerzo, además de combinarla con un **refresco** y con la existencia de una **mayor probabilidad de pedir postre**.

**2. ¿Existen preferencias alimenticias en función de la edad, género o nacionalidad? Es decir, ¿existe algún grupo más propenso a pedir un determinado tipo de comida y/o bebida?**

En relación a la primera variable hemos podido apreciar que existe un primer intervalo entre los **19 y 21 años** de comensales propensos a solicitar **comida tradicional con refresco**, mientras que el segundo intervalo entre **23 y 50 años** consumen el **mismo menú pero con zumo**. Una posible explicación puede ser que en el primer caso se trate de una cena o tentempié, mientras que la segunda situación representa un desayuno, en el que parece más sensato consumir zumo. Por otro lado, los amantes de la **comida oriental** acompañada de refresco parecen ser **menores de 27 años**.

A diferencia de la variable anterior, no hemos podido encontrar patrones relevantes en torno a las preferencias según el género de la persona. Parece ser que o bien el tipo de menú no depende del sexo o los gustos de sendos géneros son tan similares que los algoritmos no han sido capaces de extraer reglas que representen asociaciones entre estos datos.

Finalmente, en la mayoría de análisis hemos podido observar que la **nacionalidad india**, la mayoritaria de este dataset, parece tener una mayor preferencia por el **menú tradicional acompañado de zumo**. Si bien los comensales de **origen malasio** también parecen apostar por el **mismo tipo de menú**, deciden acompañarlo con un **refresco**. Este dato puede ayudar a realizar propagandas personalizadas dependiendo de la información disponible de cada comensal.

**3. ¿Qué relaciones existen entre los diferentes tipos de comidas y bebidas? ¿Cuáles son las combinaciones más y menos comunes en la encuesta?**

Parece ser que una de las combinaciones más comunes es la **comida tradicional, zumo y sin postre**, aunque como hemos observado en análisis anteriores, algunos comensales **también la consumen con un refresco**. Por otro lado, parece que la **comida oriental suele ir acompañada de un refresco**, lo cual parece más lógico si consideramos que parece servirse durante el almuerzo y que quizás por la composición de este menú puede ser más propenso a combinarse de bebidas gaseosas. 

**4. ¿Influye el tipo de comida y/o bebida elegidos para posteriormente pedir postre?**

Las hipótesis obtenidas en diferentes algoritmos sugieren que existe una **menor probabilidad** de que un comensal solicite postre si consume el **menú tradicional con zumo**, mientras que por el contrario esta **probabilidad aumenta** si lo acompaña de un **refresco**. Una posible razón explicativa puede asociarse a que la primera situación se trate de un desayuno o picoteo, mientas que la segunda represente una comida más copiosa donde existan mayores posibilidades para su consumo. Del mismo modo también ocurre con el **menú oriental combinado con un refresco**.
 
**5. ¿Existe una tendencia a pedir postre en función del momento de la comida, el género o la edad?**

Este objetivo ha sido uno de lo más complicados de estudiar en detalle puesto que parece existir una gran variedad de respuestas por parte de los encuestados. En primer lugar, parece lógico pensar que exista una **mayor probabilidad de pedir postre durante el intervalo de tarde** donde tienen lugar los almuerzos, comparado con el resto de momentos del día. Sin embargo, también parece existir **cierta probabilidad** de solicitar el tercer plato en otro momento del día, posiblemente **durante la noche**. 

Adicionalmente, parece que las **mujeres entrevistadas se encuentran más dubitativas** a la hora de decidir si tomar postre o no, mientras que las respuestas proporcionadas por los hombres son más clarificantes, siendo positivas en su mayoría. Sin embargo, más que del género también parece depender de la **nacionalidad de los comensales y del menú** previo, como hemos explicado anteriormente. 
