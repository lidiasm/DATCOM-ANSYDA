...
2. La tercera, cuarta, sexta y séptima regla contienen prácticamente el mismo tipo de información en mayor o menor detalle. Estas cuatro reglas son capaces de identificar una **dependencia entre la alimentación y el género**, puesto que con un 80% de confianza determinan que si una persona se encuentra en el intervalo de adultos, solicita comida tradicional y zumo fresco, probablemente se trate de una mujer. 

3. Finalmente la quinta regla afirma que, independientemente de las características de la persona, si se pide un plato de comida oriental es un 80% probable de que se trate de un almuerzo. Esta regla, por tanto, proporciona una primera hipótesis de los tipos de **platos que se suelen solicitar dependiendo del momento del día**.
...
```
A continuación procedemos a estudiar la variable **`Nationality` estableciéndola como el consecuente **para obtener un conjunto de reglas que intenten expresar sus dependencias con el resto de ítems. Sin embargo, tal y como podemos observar en los resultados, la **mayoría de las reglas parece no proporcionar información útil** puesto que es lógico que siendo la nacionalidad india la mayoritaria aparezca en la mayoría de los momentos del día. No obstante, la última regla sí es curiosa puesto que el intervalo de edad predominante es el joven, lo que cabría pensar que podría estar relacionado con la nacionalidad mayoritaria. Sin embargo, hasta en un 36.6% de los casos la última regla indica que si se trata de un **adulto es probable que sea de nacionalidad india**.
...
Si situamos como consecuente a los ítems de la variable `Food` podemos observar que las tres primeras reglas extraídas se corresponden con el resto de **patrones alimenticios según el momento del día**. Mientras que la comida oriental es más solicitada durante el almuerzo, **la comida tradicional es más probable por la mañana y la madrugada**. Adicionalmente, podemos observar en las otras dos reglas que existe cierta probabilidad de que este tipo de menú sea requerido por un **adulto que posteriormente no pida postre**. A diferencia de los anteriores conjuntos de reglas, el algoritmo *FP-growth* ha sido capaz de aportar información bastante útil con respecto a los patrones existentes en torno al tipo de comida que ofrecen los restaurantes considerados.
...
Finalmente procedemos a obtener un conjunto de reglas en el que el consecuente sean los ítems relacionados con la columna **`Juice`**. Tras filtrarlas según la medida de calidad *lift* podemos apreciar que existen dos reglas de elevada confianza pero bajo soporte. La primera de ellas representa la hipótesis de que si una persona visita un restaurante **por la mañana, pedirá jugo fresco**. Si bien es el tipo de bebida predominante en este dataset, no ha aparecido con los restantes momentos del día por lo que puede ser razonablemente útil. Sin embargo, es la segunda la que realmente proporciona otra pieza del patrón alimenticio que asocia este tipo de bebida con **no pedir postre**. 

Si estudiamos el conjunto completo podemos obtener la hipótesis de que si una persona se dirige hacia un restaurante por la mañana es probable que pida zumo fresco y no postre, lo que puede suponer que en el intervalo de tiempo **entre las 06AM y las 12PM el público suele ir a desayunar**.
